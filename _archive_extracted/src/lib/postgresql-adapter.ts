...
export function adaptPostgreSQLToSQLite(sql: string): string {
  ...
  // Only strip RETURNING at the end of a statement (before next ; or end of string)
...
function replaceDataTypes(sql: string): string {
  ...
  // DATE type only when used as a type (not in identifiers like hire_date)
  // Match DATE when followed by column constraint or end of column definition
  result = result.replace(/\bDATE\b(?=\s*(,|\)|$|\s+(NOT\s+NULL|PRIMARY|DEFAULT|NULL|CHECK|REFERENCES|UNIQUE)))/gi, 'TEXT');
  result = result.replace(/\bJSONB\b/gi, 'TEXT');
  result = result.replace(/\bJSON\b(?=\s*(,|\)|$|\s+(NOT\s+NULL|PRIMARY|DEFAULT|NULL|CHECK|REFERENCES|UNIQUE)))/gi, 'TEXT');
  result = result.replace(/\bUUID\b/gi, 'TEXT');
  result = result.replace(/\bBYTEA\b/gi, 'BLOB');
  result = result.replace(/\bBOOLEAN\b/gi, 'INTEGER');
  result = result.replace(/\bBOOL\b(?=\s*(,|\)|$|\s+(NOT\s+NULL|PRIMARY|DEFAULT|NULL|CHECK|REFERENCES|UNIQUE)))/gi, 'INTEGER');
