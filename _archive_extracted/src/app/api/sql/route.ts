...
export async function POST(request: NextRequest) {
  try {
    ...
    const { sql, dbType, taskId, explain } = body;
    ...
    // EXPLAIN QUERY PLAN mode
    if (explain) {
      const explainSQL = `EXPLAIN QUERY PLAN ${sql}`;
      if (taskId) {
        const task = getTaskById(taskId);
        if (task) {
          result = executeWithSchema(explainSQL, task.schema, effectiveDbType);
        } else {
          result = executeQuery(explainSQL, effectiveDbType);
        }
      } else {
        result = executeQuery(explainSQL, effectiveDbType);
      }
