...
export default function TaskPerformanceChart() {
  ...
  const avgSuccessRate = data.length > 0
    ? (data.reduce((sum, d) => sum + d.first_attempt_rate, 0) / data.length).toFixed(1)
    : '0';
          ...
          <TableBody>
            {filtered.map(entry => (
              <TableRow key={entry.task_id}>
                ...
                <TableCell>
                  <Badge variant={entry.difficulty === 'beginner' ? 'default' : entry.difficulty === 'intermediate' ? 'secondary' : 'destructive'}>
                ...
                <TableCell>{entry.avg_time_minutes > 0 ? `${entry.avg_time_minutes} ${t('analytics.minutes')}` : '—'}</TableCell>
                <TableCell>
                ...
                <TableCell>
                  <Badge variant={entry.success_rate_trend === 'improving' ? 'default' : entry.success_rate_trend === 'declining' ? 'destructive' : 'secondary'}>
