...
export default function RetentionCohorts() {
  ...
  const stats = [
    {
      label: t('analytics.retentionCohorts.avgWeek1'),
      value: `${summary.avg_week_1_rate}%`,
      icon: Users,
      color: 'text-blue-600',
    },
    {
      label: t('analytics.retentionCohorts.avgWeek4'),
      value: `${summary.avg_week_4_rate}%`,
      icon: TrendingUp,
      color: 'text-emerald-600',
    },
    {
      label: t('analytics.retentionCohorts.avgWeek8'),
      value: `${summary.avg_week_8_rate}%`,
      icon: TrendingUp,
  ...
  const retentionCurve = [
    { period: 'Week 1', rate: summary.avg_week_1_rate },
    {
      ...
      rate:
        cohorts.length > 0
          ? parseFloat((cohorts.reduce((s, c) => s + c.week_2_rate, 0) / cohorts.length).toFixed(1))
          : 0,
    },
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle>{t('analytics.retentionCohorts.retentionCurve')}</CardTitle>
        </CardHeader>
        <CardContent>
          <ResponsiveContainer width="100%" height={250}>
            <LineChart data={retentionCurve}>
              ...
              <Line
                ...
                stroke="#3b82f6"
                strokeWidth={2}
                name={t('analytics.retentionCohorts.retentionRate')}
              />
            </LineChart>
          </ResponsiveContainer>
        </CardContent>
      ...
      <Card>
        <CardHeader>
          <CardTitle>{t('analytics.retentionCohorts.cohortTable')}</CardTitle>
        </CardHeader>
        <CardContent>
                ...
                {cohorts.map((cohort) => (
                  <tr key={cohort.cohort_week} className="border-b last:border-0">
                    ...
                    <td className="text-center py-2 px-3">
                      <Badge className={getRateColor(cohort.week_1_rate)}>{cohort.week_1_rate}%</Badge>
                    </td>
                    <td className="text-center py-2 px-3">
                      <Badge className={getRateColor(cohort.week_2_rate)}>{cohort.week_2_rate}%</Badge>
                    </td>
                    <td className="text-center py-2 px-3">
                      <Badge className={getRateColor(cohort.week_4_rate)}>{cohort.week_4_rate}%</Badge>
                    </td>
                    <td className="text-center py-2 px-3">
                      <Badge className={getRateColor(cohort.week_8_rate)}>{cohort.week_8_rate}%</Badge>
                    </td>
                  </tr>
                ))}
              </tbody>
